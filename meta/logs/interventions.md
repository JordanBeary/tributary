...
## INT-019 — En-route fix: the provenance ledger's staleness check could never pass

- **Date:** 2026-09-03 (found while committing the reframe)
- **Phase:** 7 / harness · **Severity:** moderate (the check would have failed forever once wired into CI; no recorded provenance was ever wrong) · **Flag: en-route fix**
- **What the agent did:** Wrote `meta/tools/provenance_by_phase.py --check` (D17) to compare the committed table against a regeneration over the full history including HEAD. The generated table prints one row per commit *including its short hash*, so the commit that writes the table can never appear in the table it writes — a fixed point that does not exist. Running `--check` on the ten reframe commits reported STALE correctly, and would have reported STALE again immediately after every regeneration commit, forever. D17's proposal to wire `--check` into CI would therefore have failed on every push. The feasibility arithmetic here was one step of reasoning about the script's own output and was not run — the INT-014 lesson, missed in a new place.
- **Classification:** en-route fix (instruction 6; a defect in a harness tool found during the commit pass, outside the task of committing)
- **Resolution:** `--check` now compares against the history minus any *trailing* run of commits whose changed paths are a subset of the script's own outputs (`meta/provenance.md`, `meta/provenance_by_phase.svg`). The invariant becomes the honest and checkable one: **the table is current through the last commit that changed anything other than the table itself**. Only a trailing run is excluded, so a table-only commit with real work after it still reports STALE; `--write` is unchanged and always covers the full history; the check's output names the commit it verified through and how many commits it excluded. The reason is commented in the function so a later reader does not simplify the exclusion away. Verified in a throwaway repository across five states: stale before regeneration; current with the regeneration uncommitted; current with the table-only commit as HEAD (the case that could never pass before); stale again after real work; current after the next regeneration commit.
- **Consequence for D17:** the disposition the human ratified stands — the generated table is the ledger — but its check semantics are now the trailing-exclusion form, and the CI wiring remains *proposed*, not made (it also needs `fetch-depth: 0`). D17 carries a line saying so.
- **Doc changes:** `meta/tools/provenance_by_phase.py` (docstring, `GENERATED_PATHS`, `changed_paths`, `history_for_check`, `main`); D17; this entry.

## INT-020 — En-route fix: the publish workflow could not render a clean checkout

- **Date:** 2026-09-21 (found in the pre-flight check before deploying the reframed site)
- **Phase:** 7 · **Severity:** moderate (the deploy would have failed in CI; nothing wrong was ever published) · **Flag: en-route fix**
- **What the agent did:** Single-sourced the strategy memo into the site through an include of `site/tributary/embedded/_strategy_memo_body.md`, a git-ignored file written by the pre-render hook. Quarto resolves include shortcodes while it loads the project, which is before it runs the project's own pre-render step, so on a checkout where the hook has never run the include target does not exist and `quarto render site` aborts with "Include directive failed". Every local render passed because the working tree already held a copy from an earlier run; the failure shows only in a clean clone, which is what the publish workflow builds from. The page was never rendered from a clean checkout before it was committed.
- **Classification:** en-route fix (instruction 6; a defect in the publish path found while carrying out the deploy, outside the task of deploying)
- **Resolution:** `publish-site.yml` runs `sh tools/prerender.sh` from `site/` as its own step before `quarto render site`. The hook is idempotent, so Quarto running it a second time is harmless. Verified in a clean clone of HEAD: the render fails without the step and completes all ten pages with it. The reason is commented in the workflow so the step is not removed as redundant.
- **Standing consequence:** a fresh local clone needs the same first step (`cd site && sh tools/prerender.sh`) before its first render; later renders find the copy in place. A pre-publish check should render from a clean clone, not the working tree.
- **Doc changes:** `.github/workflows/publish-site.yml`; this entry.
