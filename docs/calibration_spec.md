...
| Floor-price dynamics | `floor_price` field | Distribution of floor-to-clearing ratios → sets tier floor schedule so sell-through cascades realistically (target: ~60% sell-through on fresh leads, declining by tier; the *pipeline-level* overall rate is ~49% once C19's recency penalties apply to repeat consumers) |
...
| Simulator parameter | Source signal | Fitting method |
| --- | --- | --- |
| Treatment/holdout split | `treatment` flag | Match Criteo's ~85/15 treated/control ratio; holdout receives no messages, and the effect attaches to assignment (intention-to-treat — C15) |
| Baseline response rate | `conversion` in control | Criteo's control conversion (0.19%) is recorded in the artifact but **non-transferable** (C15e, same epistemics as C9): the simulated pool's baseline is structural (~90% of contacts convert, since prospects are the 10% marketing-only dial). Criteo governs effect size and spread, not the baseline level |
...
| Acquisition channels | — (declared, C16, human-directed per P-008) | Seven-channel mix with an intent ladder (direct/organic/referral/paid search/affiliate/paid social/display): per-channel contact→application conversion targets rescaled to the structural pool rate, engagement-segment tilt, lead-quality tilt among converters, and unit economics (CPC/CPL, click→contact, CTR) emitting a monthly spend ledger (`channel_spend.parquet`) for end-to-end CAC/ROAS analysis. Table lives in `simulation/marketing.py` (`ACQ_CHANNELS`) |

**QA gates:** simulated holdout experiment, when analyzed naively, recovers the injected uplift within its own confidence interval — satisfied by construction under C15's inverse assignment and asserted sharply (within integer-rounding distance, overall and per segment) in `tests/test_marketing.py`; Qini curve on simulated data has realistic (modest) shape, not a toy staircase (checked in the Phase 4/5 analyses).
