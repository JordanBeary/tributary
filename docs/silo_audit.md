...
Status: draft v0.2, 2026-08-14 — all three silos deployed and verified (CRM per the ratified D6 trim)
Provenance: A
...
All figures are from the seeded full-scale run (`--scale 1.0 --seed 42`, regenerated 2026-08-13) and verified against the live resources after load.

| Silo | Platform | Contents | Rows | Deployed size |
| --- | --- | --- | --- | --- |
| Auction lake | S3 `s3://tributary-auction-lake-jb/auction/` | Hive-partitioned Parquet, `event_date=YYYY-MM-DD/`, 366 partitions, 1,098 objects | 25,024,926 events | 1,226,209,268 bytes (byte-exact vs. local) |
| CRM | Neon Postgres, `leads` table (D6 shape: no street/city, hash as `BYTEA`) | full snapshot, entity grain | 2,279,540 | 0.462 GB relation; 0.470 GB database logical |
| Marketing | BigQuery `marketing` dataset | `contacts`, `messages` (partitioned by `DATE(sent_at)`, clustered by `channel`), `channel_spend` | 1,586,849 / 4,045,057 / 84 | 0.168 / 0.416 / ~0 GB logical |

Auction event mix: 8,343,564 `bid_request`, 8,337,798 `bid`, 6,871,260 `no_sale`, 1,472,304 `win`; 2,423,428 distinct `lead_uuid`s; window 2025-07-01 through 2026-07-01.

Queryability from VS Code (the Phase 2 exit check):

- S3: local DuckDB with `httpfs` + the `tributary` profile reads the lake directly; a one-month slice (October 2025) returns 712,970 bid requests and a $197.49 mean clearing price on wins in one query.
- BigQuery: the benchmark query in Section 3 runs from the local Python client under ADC.
- Neon: the deployed `leads` table answers funnel queries from local `psycopg2` — 1,188,557 sold / 907,788 closed_lost / 183,195 funded (funded/sold = 13.4%, matching the C17d artifact rate).

The crosswalk (`data/private/crosswalk.parquet`, 369 MB) was uploaded nowhere; the S3 loader refuses to run if any crosswalk artifact appears under its source tree, and the deployed object listing contains only `auction/` keys.
...
2. **"How many distinct consumers do we auction?"** — answered wrongly. 2.42M `lead_uuid`s overcount consumers: ~8% of consumers apply multiple times under varied identity data, and each application is a fresh UUID. The lake cannot see that two leads are one person.
...
3. **"How many unique applicants do we have?"** — answered wrongly, twice over. Duplicate applications (~8%) inflate the count with nickname/typo variants, and the pre-migration window is silently missing ~5% of legacy records (C17c) while their auction events live on as orphans — so the CRM both overcounts recent applicants and undercounts history.
...
- **S3 load**: 1,098 PUT-class requests (plus multipart overhead) — well under $0.02 one-time; ~1.23 GB standard storage ≈ $0.03/month. The few-large-files layout was uploaded once; the writes-cost-~12x-reads rule (design Section 5) is why the loader syncs rather than iterates remotely.
