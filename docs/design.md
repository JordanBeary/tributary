...
4. **`generate_marketing`** — pre-submission nurture campaigns (email/SMS sends, opens, clicks) with a randomized holdout flag and a small true uplift effect on application probability, calibrated to Criteo Uplift effect sizes. ~4M messages. Contacts enter the pool through a declared mix of acquisition channels (organic search, paid search, display, paid social, affiliate, referral, direct) with an intent ladder — organic converts best, display worst — driving per-channel funnel KPIs, lead quality, and a monthly spend ledger, so unified ROAS-by-channel is computable end to end (C16).
