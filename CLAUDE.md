...
Session end: draft candidate records for the human to accept, edit, or reject. Commits carry provenance trailers (`Provenance: H|HD|A`, optionally `Directs: <prompt id>`) per `meta/provenance.md`. Commits in which an agent did work also carry `Models-Used: <model id>` and `Co-Authored-By: <model name> <noreply address>` trailers, after the provenance trailers (D20, 2026-09-21; this supersedes INT-010's prohibition). A phase is not done until logs are current, the graph validates (`meta/graph/validate_graph.py`), and provenance is recorded.
...
- **S3 uploads on this network need small multipart chunks.** Something on-path deterministically resets single-request PUTs of certain payloads ("Could not connect to the endpoint URL" for the same files across runs, while same-size random bytes pass). `~/.aws/config` pins the `tributary` profile to 1 MB multipart threshold/chunksize + adaptive retries (max 10) — do not "simplify" that config away; without it some uploads fail deterministically (D5e, 2026-08-13).
